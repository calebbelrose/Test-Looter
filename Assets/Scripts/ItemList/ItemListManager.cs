...
public class ItemListManager : MonoBehaviour {
    ...
    private void Update()
    {
        if (Input.GetMouseButtonDown(1) && InvenGridManager.Instance.selectedButton != null) //right click to return item to list if item is from list
        {
    ...
    {
        if (InvenGridManager.Instance.selectedButton == null && ItemScript.selectedItem != null) //add item to list if item is not from list
        {
            ItemClass item = ItemScript.selectedItem.item;
