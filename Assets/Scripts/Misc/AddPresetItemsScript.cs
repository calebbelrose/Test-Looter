...
public class AddPresetItemsScript : MonoBehaviour {
    ...
    public void RemoveGridItems()
    {
        ...
        IntVector2 gridSize = InvenGridManager.Instance.gridSize;
        GameObject[,] slotGrid = InvenGridManager.Instance.slotGrid;
        SlotScript instanceScript;
        for (int y = 0; y < gridSize.y; y++)
        {
            for (int x = 0; x < gridSize.x; x++)
            {
                slotGrid[x, y].GetComponent<Image>().color = Color.white;
